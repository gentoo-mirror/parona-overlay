...
inherit desktop multiprocessing pax-utils unpacker xdg
...
src_unpack() {
	unsquashfs -no-progress -p $(makeopts_jobs) -d "${S}" "${DISTDIR}/${P}.snap" \
		/usr/share/doc/spotify-client /usr/share/spotify || die
}
